...
import math
import copy
from dataset import ValOrchidDataset,CompOrchidDataset
...
def set_environment(args,size = 600):
    ...
    test_set = CompOrchidDataset(args.data_root,val_transforms) # data_transform
...
def test(args, model, test_loader):
    ...
    pbar.close()
    if args.vote:
        output_file = "./Vote_part_submission.csv"
        if os.path.exists(output_file):
            df = pd.read_csv(output_file)
        else:   
            df = pd.read_csv('Orchid_Final_ds/submission_template.csv')
...
if __name__ == "__main__":
    ...
    if args.vote:
        # vote1
        args.pretrained_path = 'backup/best_94_97.pth'
        test_loader, model = set_environment(args)
        test(args, model, test_loader)
        # vote2
        args.pretrained_path = 'backup/best_510_94_97.pth'
        test_loader, model = set_environment(args,size = 510)
        test(args, model, test_loader)
        # vote3
        args.pretrained_path = 'backup/best_birds_94_97.pth'
    ...
    if args.tv:
        answer = []
        correct = 0
        df = pd.read_csv('./Vote_part_submission.csv')
        num = len(df['filename'])
